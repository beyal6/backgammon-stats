...
const PLAYERS: Player[] = ['אייל', 'הינס']
...
export function AddGame({ onAdd }: Props) {
  ...
  const loser = winner ? (PLAYERS.find((p) => p !== winner) as Player) : null

  async function handleSubmit() {
    if (!winner || !points || !loser) return
    setLoading(true)
    try {
    ...
    <div className="flex flex-col gap-6 pb-24">
      <div className="bg-surface-800 ring-1 ring-white/10 rounded-2xl p-6 flex flex-col gap-6">
              ...
              <button
                ...
                className={`py-6 rounded-2xl text-2xl font-bold transition-all ${
                  winner === p
                    ...
                    : 'bg-surface-900 text-slate-300 hover:bg-surface-800 hover:text-white'
                }`}
              >
                {p}
              </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            {POINTS.map((pt) => (
              ...
              >
                ...
                <span className="text-xs opacity-80">{POINT_LABELS[pt]}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {winner && points && (
          <div className="bg-indigo-600/10 ring-1 ring-indigo-500/30 rounded-xl p-4 text-center">
            <p className="text-white">
              <span className="font-bold text-indigo-300">{winner}</span> ניצח ·{' '}
              <span className="text-slate-400">{POINT_LABELS[points]}</span> ({points} נק׳)
            </p>
          </div>
        )}
        ...
        <button
          ...
          disabled={!winner || !points || loading}
          className={`w-full py-4 rounded-2xl text-lg font-bold transition-all ${
            winner && points && !loading
